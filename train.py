...
def train(train_loader, model, optimizer, epoch, start_iter, cfg):
    ...
    for iter, data in enumerate(train_loader):
        ...
        loss_loc = torch.mean(outputs['loss_loc'])
        losses_loc.update(loss_loc.item())

        loss =  loss_kernels + loss_loc
        ...
        if iter % 20 == 0:
            output_log = '({batch}/{size}) LR: {lr:.6f} | Batch: {bt:.3f}s | Total: {total:.0f}min | ' \
                         ...
                         'Loss(kernel/loc): {loss_kernel:.3f}/{loss_loc:.3f} ' \
                         '| IoU(text/kernel): {iou_text:.6f}/{iou_kernel:.6f} '.format(
                batch=iter + 1,
...
def save_checkpoint(state, checkpoint_path, cfg):
    ...
    if cfg.data.train.type in ['synth'] or \
            (state['iter'] == 0 and state['epoch'] % 10 == 0):
